...
# --- Thresholds ---
BLINK_THRESH  = 0.18
BLINK_HOLD_SEC = 0.3   # how long to hold space after a blink
MAR_THRESH    = 5       # pixels
COOLDOWN_SEC  = 0.2     # seconds between triggers for mouth/blink
...
def cv_loop():
    ...
    while not stop_event.is_set():
        ...
        # ── Slam mode ────────────────────────────────────────────────────────
        if modes["slam_press"]:
            if keyboard.is_pressed('space') and (now - last_slam_time) > COOLDOWN_SEC:
                pyautogui.press('w')
                last_slam_time = now
        ...
        if result.multi_face_landmarks:
            ...
            # Mouth open (fires on open → closed transition)
            if modes["mouth_press"]:
                mar = mouth_open_distance(lm, 13, 14, img_w, img_h)
                mouth_open = mar > MAR_THRESH

                if mouth_open:
                    if (now - last_mouth_time) > COOLDOWN_SEC:
                        pyautogui.press('space')
                        last_mouth_time = now
                        print(f"Mouth open → spacebar (MAR={mar}px)")
